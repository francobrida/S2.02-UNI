...
WHERE tipo = 'alumno' ORDER BY apellido1, apellido2, nombre ASC;
...
SELECT id, nombre, cuatrimestre, id_grado FROM asignatura
...
SELECT departamento.nombre AS nombre, persona.apellido1, persona.apellido2, persona.nombre FROM persona
JOIN profesor ON persona.id = profesor.id_profesor
...
SELECT COUNT(persona.id) AS total FROM persona
WHERE tipo = 'alumno';
...
SELECT departamento.nombre AS departamento,COUNT(profesor.id_departamento) AS total FROM departamento
JOIN profesor ON departamento.id = profesor.id_departamento 
GROUP BY departamento.nombre ORDER by total desc;
...
SELECT grado.nombre AS grado, COUNT(asignatura.id) AS total FROM grado
LEFT JOIN asignatura ON grado.id = asignatura.id_grado
GROUP BY grado ORDER BY total DESC;
...
SELECT DISTINCT grado.nombre AS grado, asignatura.tipo AS tipo, SUM(asignatura.creditos) AS total_creditos FROM grado
LEFT JOIN asignatura ON grado.id = asignatura.id_grado
GROUP BY grado, asignatura.tipo;
...
SELECT anyo_inicio, count(DISTINCT alumno_se_matricula_asignatura.id_alumno) AS total FROM curso_escolar
...
JOIN profesor ON persona.id = profesor.id_profesor
LEFT JOIN asignatura ON profesor.id_profesor = asignatura.id_profesor
GROUP BY persona.id ORDER BY total DESC;

-- 25. Retorna totes les dades de l'alumne/a més jove. (*)
SELECT nombre, apellido1, apellido2, fecha_nacimiento FROM persona WHERE tipo = 'alumno' 
